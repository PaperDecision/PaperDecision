...
def pdf_pages_to_resized_base64(pdf_path, target_width=1280, target_height=720, fmt="JPEG"):
    """
    Convert each PDF page to an image at a target resolution, then encode as Base64.

    Args:
        pdf_path: PDF file path
        target_width: Target width (pixels). If provided, scale by width (preferred)
        target_height: Target height (pixels). If provided, scale by height (used if width is not set)
        fmt: Image format (PNG/JPG)

    Returns:
        List of Base64-encoded strings, one per page image
    """
    try:
        if not os.path.exists(pdf_path):
            raise FileNotFoundError(f"PDF file does not exist: {pdf_path}")
        
        # Convert using default DPI (200) to get initial images/sizes.
        # Note: DPI here only affects the initial render; final size is controlled by target_width/height.
        images = convert_from_path(pdf_path, dpi=200, fmt=fmt.lower())
        # print(f"PDF has {len(images)} pages; resizing to the target resolution...")
        ...
        for img in images[:10]:
            ...
            # Compute scaling factor (prefer width; if not set, use height)
            if target_width:
                scale = target_width / orig_width
            elif target_height:
                scale = target_height / orig_height
            else:
                # If neither is set, keep original size
                scale = 1.0
...
url = ''
app_key = os.environ.get('GPTAppKey', 'gpt-')        # Replace this with your APPKey
if app_key == '':
    ...
    for attempt in range(max_retries):
        try:
            data = {
                ...
                "stream": False,
                # "ai_studio": True,
                "generationConfig": config_args
            }
            response = requests.post(used_base_url, json=data, headers=headers, timeout=60)
